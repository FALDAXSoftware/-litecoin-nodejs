      ...
      node(label) {
         ...
         container('build-container'){
              ...
              if (namespace) {
                  ...
                  s3Download(file:'.env', bucket:'env.faldax', path:"litecoin-node-js/${namespace}/.env", force:true)
                }
                sh "ls -a"
                sh "docker build -t ${imageRepo}/litecoin:${imageTag}  ."
                sh "docker push  ${imageRepo}/litecoin:${imageTag}"
                sh "helm upgrade --install --namespace ${namespace} --set image.tag=${imageTag} ${namespace}-litecoin-backend -f chart/values-${namespace}.yaml chart/"
              }
         }
         }
